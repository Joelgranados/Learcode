...
for boost_rtopt_ in $boost_rtopt '' -d; do
  ...
  do
    ...
    do
      ...
      bmaj=`echo $boost_ver_ | sed "s/-//g;" | cut -d'_' -f 1`
      bmin=`echo $boost_ver_ | sed "s/-//g;" | cut -d'_' -f 2`
      if test x"$bmaj" != x ; then
          if test "$bmaj" -ge 1 -a "$bmin" -gt 34 ; then
          `echo $Boost_lib_LIBS | grep -q boost_system `;
              if test $? -gt 0 ; then
                  Boost_lib_LIBS="$Boost_lib_LIBS ${Boost_lib_LIBS/boost_$1/boost_system}"
              fi
          fi
      fi
...
AC_LANG_POP([C++])dnl
  ...
  case $boost_cv_lib_tag in #(
    ...
    gcc*)
      ...
      boost_cv_lib_tag="$boost_cv_lib_tag -x$boost_cv_lib_tag -gcc -xgcc"
      ;; #(
    unknown)
